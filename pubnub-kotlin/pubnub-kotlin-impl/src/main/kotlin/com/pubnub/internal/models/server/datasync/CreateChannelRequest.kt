...
internal data class CreateChannelRequest(
    val data: CreateChannelRequestData,
)

internal data class CreateChannelRequestData(
    // Omitted from the wire payload when null (server generates the id) — the SDK's Gson instance
    // does not serialize nulls.
    val id: String? = null,
    // Omitted from the wire payload when null — the server defaults it to "Channel". When set it must
    // be a `Channel` subclass.
