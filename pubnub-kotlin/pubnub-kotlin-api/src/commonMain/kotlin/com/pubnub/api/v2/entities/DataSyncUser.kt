...
/**
 * A handle for subscribing to DataSync realtime events about a User by its ref/id.
 *
 * A DataSync handle is a **channel filter on the ref**, not a type filter: subscribing to
 * `dataSyncUser(id)` delivers the mix of events routed to that ref-channel — the user's own
 * create/update/delete, the memberships/relationships where the user is an endpoint (published to both
 * endpoint refs, never to a membership/relationship-id channel), and propagated events for entities
 * joined to it. Branch on the sealed
 ...
 * is separate from the DataSync CRUD permission on the user *record*
 * ([com.pubnub.api.models.consumer.access_manager.v3.UserGrant]): the record grant does not authorize the
 * subscribe, and the channel `read` grant does not authorize record CRUD.
 *
 * Use [com.pubnub.api.PubNub.dataSyncUser] to create instances.
 */
interface DataSyncUser : Subscribable {
    /**
     * The user's ref/id.
     */
    val id: String
    ...
    /**
     * Returns a [Subscription] to this user's default-projection ref-channel.
     */
