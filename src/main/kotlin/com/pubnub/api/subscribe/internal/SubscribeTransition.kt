...
fun subscribeTransitions(shouldRetry: (Int) -> Boolean): SubscribeTransitions =
    transitions { state, event ->
        when (state) {
            is Handshaking -> when (event) {
                is Commands.SubscribeIssued -> transitionTo(
                    target = Handshaking(updatedStatus),
                    onExit = cancel(state.retryWrapping, state.call)
                )
            ...
            is Receiving -> {
                when (event) {
                    ...
                    is Commands.UnsubscribeIssued -> {
                        transitionTo(
                            target = Receiving(updatedStatus),
                            onExit = cancel(state.call)
                        )
                    }
                    ReceivingResult.ReceivingFailed -> transitionTo(Reconnecting(updatedStatus))
