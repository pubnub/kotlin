...
interface SubscribeCapable {
     ...
     * or will alter the existing connection to add channels and groups requested by this subscription if needed.
     *
     * Please note that passing a [cursor] to [subscribe] affects *all* subscriptions that are currently active in the
     * [com.pubnub.api.PubNub] client, as it will reset the global timetoken for the server connection. If a
     * subscription previously delivered *any* events to its listeners, it will only deliver events *newer* that the
     * last timetoken it recorded, even if the new timetoken is older.
     */
