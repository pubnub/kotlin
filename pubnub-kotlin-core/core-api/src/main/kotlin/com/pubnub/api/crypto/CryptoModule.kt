...
private fun instantiateCryptoModuleImpl(primaryCryptor: Cryptor, cryptorsForDecryptionOnly: List<Cryptor>) : CryptoModule {
    return Class.forName("com.pubnub.internal.crypto.CryptoModuleImpl").getConstructor(Cryptor::class.java, List::class.java).newInstance(
        primaryCryptor, cryptorsForDecryptionOnly
    ) as CryptoModule
}

private fun instantiateLegacyCryptor(cipherKey: String, randomIv: Boolean = true): Cryptor {
    return Class.forName("com.pubnub.internal.crypto.cryptor.LegacyCryptor").getConstructor(String::class.java, Boolean::class.java).newInstance(
        cipherKey, randomIv
    ) as Cryptor
}
