...
abstract class BasePubNub internal constructor(val configuration: PNConfiguration, eventEnginesConf: EventEnginesConf) : EventEmitter, StatusEmitter {
    ...
    inner class PubNubImpl internal constructor(
        ...
        fun subscribe(
            ...
            channels.map { ChannelName(it) }.forEach { channelName ->
                ...
                channelSubscriptionMap.computeIfAbsent(channelName) { newChannelName ->
                    val channel = ChannelImpl(this@BasePubNub, newChannelName)
                ...
                if (withPresence) {
                    channelSubscriptionMap.computeIfAbsent(channelName.withPresence) { presenceChannelName ->
                        ...
                        subscription ?: ChannelImpl(this@BasePubNub, presenceChannelName).subscription().also { sub ->
                            toSubscribe.add(sub)
                        }
                    }
                }
            ...
            channelGroups.map { ChannelGroupName(it) }.forEach { channelGroupName ->
                ...
                channelGroupSubscriptionMap.computeIfAbsent(channelGroupName) { newChannelGroupName ->
                    val channelGroup = ChannelGroupImpl(this@BasePubNub, newChannelGroupName)
                    val options =
                        if (withPresence) {
                            SubscriptionOptions.receivePresenceEvents()
                        } else {
                ...
                if (withPresence) {
                    channelGroupSubscriptionMap.computeIfAbsent(channelGroupName.withPresence) { presenceGroupName ->
                        // this will either be the subscription we just created in the previous step,
                        // or if we were already subscribed to the channel WITHOUT presence, we need to create a new one
                        subscription ?: ChannelGroupImpl(this@BasePubNub, presenceGroupName).subscription().also { sub ->
                            toSubscribe.add(sub)
                        }
                    }
                }
        ...
        fun channel(name: String): Channel {
            return ChannelImpl(this@BasePubNub, ChannelName(name))
        }
        ...
        fun channelGroup(name: String): ChannelGroup {
            return ChannelGroupImpl(this@BasePubNub, ChannelGroupName(name))
        }
        ...
        fun channelMetadata(id: String): ChannelMetadata {
            return ChannelMetadataImpl(this@BasePubNub, ChannelName(id))
        }
        ...
        fun userMetadata(id: String): UserMetadata {
            return UserMetadataImpl(this@BasePubNub, ChannelName(id))
        }
        ...
        fun subscriptionSetOf(subscriptions: Set<Subscription> = emptySet()): SubscriptionSet {
            ...
            return SubscriptionSetImpl(this@BasePubNub, subscriptions as Set<SubscriptionImpl>)
        }
        ...
        fun subscriptionSetOf(
            ...
            val subscriptionSet = SubscriptionSetImpl(this@BasePubNub)
            channels.forEach {
