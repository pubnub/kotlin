...
import org.junit.jupiter.api.Test
 ...
 class BaseSubscriptionImplTest {
    ...
    fun setUp() {
        ...
        subscription = object : BaseSubscriptionImpl<TestEventListener>(pubnub.pubNubCore, setOf(ChannelName(channelName))) {
    ...
    fun `subscriptions with filter`() {
        // given
        val subWithFilter = object : BaseSubscriptionImpl<TestEventListener>(pubnub.pubNubCore, setOf(ChannelName(channelName)), options = SubscriptionOptions.filter { it !is PNMessageResult }) {
            override fun addListener(listener: TestEventListener) {
    ...
    fun `subscriptions shouldn't get events until subscribe is called`() {
        // given
        subscription.addListener(TestEventListener { throw IllegalStateException("Message should not be received without subscribe call!") })
