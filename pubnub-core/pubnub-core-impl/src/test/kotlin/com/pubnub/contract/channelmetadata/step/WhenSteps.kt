...
class WhenSteps(
    ...
    fun i_get_the_channel_metadata() {
        world.pubnub.corePubNubClient.getChannelMetadata(channel = channelMetadataState.channelId).sync()?.let {
    ...
    fun i_set_the_channel_metadata() {
        ...
        world.pubnub.corePubNubClient.setChannelMetadata(
    ...
    fun i_get_the_channel_metadata_with_custom() {
        world.pubnub.corePubNubClient.getChannelMetadata(
    ...
    fun i_remove_the_channel_metadata() {
        world.responseStatus =
            world.pubnub.corePubNubClient.removeChannelMetadata(
                channel = channelMetadataState.channelId,
            ).sync().status
    }
    ...
    fun i_get_all_channel_metadata() {
        world.pubnub.corePubNubClient.getAllChannelMetadata().sync().let {
    ...
    fun i_get_all_channel_metadata_with_custom() {
        world.pubnub.corePubNubClient.getAllChannelMetadata(
            includeCustom = true,
        ).sync().let {
            channelMetadataState.channelMetadatas = it.data
            world.responseStatus = it.status
